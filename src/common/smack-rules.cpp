...
bool SmackRules::addFromTemplateFile(const std::string &appId,
        const std::string &pkgId, const std::vector<std::string> &pkgContents)
{
    ...
    return addFromTemplate(templateRules, appId, pkgId, pkgContents);
}

bool SmackRules::addFromTemplate(const std::vector<std::string> &templateRules,
        const std::string &appId, const std::string &pkgId, const std::vector<std::string> &pkgContents)
{
    for (auto rule : templateRules) {
        ...
        if (subject == SMACK_APP_LABEL_TEMPLATE) {
            if (!generatePkgIdLabel(appId, subject)) {
        ...
        if (object == SMACK_PKG_LABEL_TEMPLATE) {
            if (!generateAppIdLabel(pkgId, object)) {
                LogError("Failed to generate pkg label from pkgId: " << pkgId);
                return false;
            }
        }
...
bool SmackRules::generatePackageCrossDeps(const std::string &pkgId, const std::vector<std::string> &pkgContents)
{
    ...
    for (const auto &subject : pkgContents) {
        for (const auto &object : pkgContents) {
            ...
            if (generateAppIdLabel(subject, subjectLabel) && generateAppIdLabel(object, objectLabel)) {
...
{
    std::string path(tzplatform_mkpath3(TZ_SYS_SMACK, "accesses.d", std::string ("pkg_").append (pkgId).c_str()));
    return path;
}

std::string SmackRules::getApplicationRulesFilePath(const std::string &appId)
{
    std::string path(tzplatform_mkpath3(TZ_SYS_SMACK, "accesses.d", std::string ("app_").append (appId).c_str()));
    return path;
}
...
bool SmackRules::installApplicationRules(const std::string &appId, const std::string &pkgId,
    ...
    try {
        ...
        if (!smackRules.addFromTemplateFile(appId, pkgId, pkgContents)) {
...
bool SmackRules::updatePackageRules(const std::string &pkgId, const std::vector<std::string> &pkgContents)
{
    try {
        ...
        if (!smackRules.generatePackageCrossDeps(pkgId, pkgContents))
        {
...
bool SmackRules::uninstallApplicationRules(const std::string &appId, const std::string &pkgId, std::vector<std::string> pkgContents)
{
    std::string path = getApplicationRulesFilePath(appId);
    if (access(path.c_str(), F_OK) == -1) {
        if (errno == ENOENT) {
            LogWarning("Smack rules were not installed for appId: " << pkgId);
            return true;
        }
    ...
    try {
        ...
        if (rules.loadFromFile(path)) {
            if (smack_smackfs_path() != NULL && !rules.clear()) {
                LogWarning("Failed to clear smack kernel rules for appId: " << appId);
                // don't stop uninstallation
            }
        } else {
            LogWarning("Failed to load rules from file: " << path);
        ...
        LogError("Out of memory while trying to uninstall smack rules for appId: " << appId);
        return false;
    }
}
} // namespace SecurityManager
