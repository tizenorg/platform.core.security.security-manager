...
static int label_for_self(const std::string &label)
{
    ...
    snprintf(buf, 512, " /proc/self/task/%d/attr/current", gettid());
...
static int syncThreadsSecurityCtx(const char *app_name)
{
    ...
    act.sa_handler = [](int signo) {
        /*
         * Should we block any other signal here?
         */
        LogDebug("Got signal nr: " << signo);
        std::atomic_thread_fence(std::memory_order_acquire);
        label_for_self(SecurityManager::SmackLabels::generateAppLabel(g_app_name));
        if (security_manager_drop_process_privileges())
            LogError("Can't drop privileges");
        g_threads_count--;
    };
    ...
    for (auto const &e : files) {
        ...
        /*
         * g_threads_count is decremented during signal handler
         * we should block SIGUSR1 during incrementation in main thread
         */
    ...
    for (int i = 0; g_threads_count != 0 && i < 1000; ++i)
        usleep(1000);

    sigaction(SIGUSR1, &old, nullptr);

    if (g_threads_count != 0) {
        LogError("Not all threads synced with security ctx! Threads left: " << g_threads_count);
        return SECURITY_MANAGER_ERROR_SERVER_ERROR;
    }
...
int security_manager_prepare_app(const char *app_name)
{
    ...
    if (ret != SECURITY_MANAGER_SUCCESS) {
        LogError("Can't drop process privileges");
        return ret;
    }
