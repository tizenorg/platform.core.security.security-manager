...
static std::map <std::string, enum app_install_path_type> app_install_path_type_map = {
    ...
    {"invalid", SECURITY_MANAGER_ENUM_END}
};

static app_install_path_type
string_to_app_install_path_type(const std::string &pathType)
{
    try {
        return app_install_path_type_map.at(pathType);
    } catch (const std::out_of_range &e) {
        LogError("Failed to convert '" << pathType <<
                 "' to app_install_path_type");
        return SECURITY_MANAGER_ENUM_END;
    }
}
                      ...
                      struct app_inst_req &req)
{
    ...
    for (std::vector<std::string>::size_type i = 0; i < paths.size(); ++i) {
        ...
        if (i & 1) {
            ...
            pathType = string_to_app_install_path_type(paths[i]);
