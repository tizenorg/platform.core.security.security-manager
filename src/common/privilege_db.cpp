...
void PrivilegeDb::GetPrivilegeVersions(const std::string &privilege, std::vector<std::string> &versions)
{
    try_catch<void>([&] {
        auto &command = getStatement(StmtType::EGetPrivilegeVersions);
        command->BindString(1, privilege);

        versions.clear();
        while (command->Step()) {
            std::string version = command->GetColumnString(0);
            LogDebug("Privilege " << privilege << " has version " << version);
                                       ...
                                       std::vector<std::string> &mappings)
{
    try_catch<void>([&] {
        ...
        while (command->Step()) {
            std::string mapping = command->GetColumnString(0);
            LogDebug("Default mapping from version " << version_from
