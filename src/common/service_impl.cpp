...
bool ServiceImpl::getUserAppDir(const uid_t &uid, const int &installType, std::string &userAppDir)
{
    ...
    if (static_cast<app_install_type>(installType) == app_install_type::SM_APP_INSTALL_LOCAL)
        id = TZ_USER_APP;
    else if (static_cast<app_install_type>(installType) == app_install_type::SM_APP_INSTALL_GLOBAL)
        id = TZ_SYS_RW_APP;
    else
        id = TZ_SYS_RO_APP;
...
bool ServiceImpl::installRequestAuthCheck(const app_inst_req &req, uid_t uid, std::string &appPath)
{
    ...
    if (!getUserAppDir(uid, req.installationType, userAppDir)) {
