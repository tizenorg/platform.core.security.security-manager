...
bool ServiceImpl::getUserAppDir(const uid_t &uid, const app_install_type &installType, std::string &userAppDir)
{
    ...
    auto tz_ctxPtr = unique_ptr_wrap(tz_ctx, &tzplatform_context_destroy);
    ...
    auto real_pathPtr = unique_ptr_wrap(realpath(appDir, NULL), &free);
    if (!real_pathPtr.get()) {
...
bool ServiceImpl::installRequestPathsCheck(const app_inst_req &req, std::string &appPath)
{
    ...
    for (const auto &path : req.appPaths) {
        auto real_path = unique_ptr_wrap(realpath(path.first.c_str(), NULL), &free);
        if (!real_path.get()) {
