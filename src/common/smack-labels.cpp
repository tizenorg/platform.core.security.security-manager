...
std::string getSmackLabelFromPid(pid_t pid)
{
    ...
    if (smack_label_length(result.c_str()) <= 0)
        ThrowMsg(SmackException::InvalidLabel, "Unable to fetch Smack label for process " << pid);
...
std::string getSmackLabelFromSelf(void)
{
    ...
    if (labelSize <= 0)
        ThrowMsg(SmackException::InvalidLabel, "Unable to fetch Smack label for current process");
