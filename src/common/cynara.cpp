...
Cynara::Cynara()
{
    ...
    checkCynaraError(
        cynara_async_initialize(&cynara, nullptr, &Cynara::statusCallback, &(pollFds[0])),
...
void Cynara::threadEventPut(eventfd_t event)
{
    int ret = eventfd_write(pollFds[0].fd, event);

    if (ret == -1)
        LogError("Unexpected error while writing to eventfd: " << strerror(errno));
}

eventfd_t Cynara::threadEventGet()
{
    eventfd_t event;
    int ret = eventfd_read(pollFds[0].fd, &event);
    if (ret == -1)
    ...
    void *ptr)
{
    ...
    LogDebug("Cynara status callback. " <<
        "Status = " << status << ", oldFd = " << ", newFd = " << newFd);
...
void Cynara::run()
{
    ...
    while (true) {
        ...
        if (pollFds[0].revents && threadEventGet() == eventTerminate)
            break;
        ...
        const std::string &user, const std::string &session)
{
    ...
    // Critical section
    {
        ...
        threadEventPut(eventPollRestart);
