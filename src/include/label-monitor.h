 ...
 * @version     1.0
 * @brief       This file contains header of API for managing list of permited labels for launcher
 */
#ifndef SRC_INCLUDE_LABEL_MONITOR_H_
#define SRC_INCLUDE_LABEL_MONITOR_H_
 ...
 * \ref security_manager_app_inst_labels_monitor_finish.
 ...
 *     struct pollfd fds[1];
 *
 *     security_manager_app_inst_labels_monitor_init(&monitor);
 *     security_manager_app_inst_labels_process(monitor);
 *     security_manager_app_inst_labels_monitor_get_fd(monitor, &fd);
 *     fds[0].fd = fd;
 *     fds[0].events = POLLIN;
 *     while (1) {
 ...
 *     security_manager_app_inst_labels_monitor_finish(monitor);
 *
 * \endcode
 */
 ...
 * Frees all resources previously allocated by \ref security_manager_app_inst_labels_monitor_init
 *
...
#endif /* SRC_INCLUDE_LABEL_MONITOR_H_ */
