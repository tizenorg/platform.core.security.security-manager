...
int ServiceImpl::appInstall(const app_inst_req &req, uid_t uid)
{
    ...
    try {
        ...
        // TODO something looks wrong here... why we call cynara twice???
        CynaraAdmin::getInstance().UpdateAppPolicy(appLabel, uidstr, oldAppPrivileges, req.privileges);
...
>>>>>>> parent of f868403... Implement master and slave mode
{
    if (uid != 0)
        return SECURITY_MANAGER_API_ERROR_AUTHENTICATION_FAILED;
