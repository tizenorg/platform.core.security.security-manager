        ...
        const std::string &user_label_file)
{
    ...
    try {
        PermissibleSet::readLabelsFromPermissibleFile(global_label_file, labels);
        PermissibleSet::readLabelsFromPermissibleFile(user_label_file, labels);
        std::vector<const char*> temp;
        std::transform(labels.begin(), labels.end(), std::back_inserter(temp),
                [] (std::string &label) {label = SmackLabels::generateAppLabel(label); return label.c_str();});
        ...
        LogDebug("Invalid state of configuration files - smack_set_relabel_self not called");
...
int security_manager_app_labels_monitor_init(app_labels_monitor **monitor)
{
    ...
    return try_catch([&] {
        ...
        int ret;
        lib_retcode ret_lib;
        const std::string globalFile = PermissibleSet::getPerrmissibleFileLocation(SM_APP_INSTALL_GLOBAL);
        const std::string userFile = PermissibleSet::getPerrmissibleFileLocation(SM_APP_INSTALL_LOCAL);
