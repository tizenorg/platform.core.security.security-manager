...
int security_manager_app_labels_monitor_init(app_labels_monitor **monitor)
{
    ...
    return try_catch([&] {
        ...
        int ret;
        lib_retcode ret_lib;
        const std::string user_smack_apps_label_file =
        ...
        *monitor = nullptr;
        ...
        ret_lib = inotify_add_watch_full(m.get()->inotify, global_apps_label_file.c_str(),
                IN_MODIFY, &(m.get()->global_labels_file_watch));
...
int security_manager_app_labels_monitor_process(app_labels_monitor *monitor)
{
    ...
    return try_catch([&] {
        ...
        const std::string user_smack_apps_label_file = Config::SMACK_APPS_LABELS_USER_FILE_PREFIX +
