 ...
 *  to the given entry using privacy manager or device admin tool. The second one shows the current
 *  state of this permission. Both max_value and current fields correspond to Cynara policy result.
 */
struct policy_entry {
 ...
 *         of given field. Use it, for example when it is desired to apply policy change for all
...
/**
 * \brief Function fetches all privileges that are edited by admin user.
...
/**
 * \brief Function fetches all privileges that are edited by user in his privacy manager.
 *        The result is stored in the policy_entry structures array.
 *
 * It uses dynamic allocation inside and user responsibility is to call
 * security_manager_policy_entries_free() for freeing allocated resources.
...
/**
 * \brief Function gets whole policy for the user specified in argument. The result is stored in
 *        the policy_entry structures array.
 *
 * \attention It should be called by admin user. Normal users may list policy of privileges
 *            and applications using security_manager_get_whole_policy_for_self() API function.
 *
 ...
 * \return API return code or error code
 */
int security_manager_get_whole_policy_for_user(const uid_t uid,
