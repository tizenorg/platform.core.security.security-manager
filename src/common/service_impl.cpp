...
int ServiceImpl::appUninstall(const Credentials &creds, app_inst_req &&req)
{
    ...
    try {
        ...
        if (authorId != -1 && removeAuthor) {
            ...
            } catch (const SmackException::FileError&) {
                /*
                 * TODO: If the uninstall request doesn't contain proper
                 * authorName the service will try to revoke rules basing on
                 * package author. However, if it's not equal to author from
                 * installation it will fail here. For now it's ignored.
                 */
                LogWarning("There's no author rules file for app: " <<
                           req.appName << ", authorId: " << authorId);
            }
        }
