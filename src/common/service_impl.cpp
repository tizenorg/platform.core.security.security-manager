...
int ServiceImpl::getPolicy(const policy_entry &filter, uid_t uid, pid_t pid, const std::string &smackLabel, std::vector<policy_entry> &policyEntries)
{
    try {
        ...
        for (const uid_t &user_id : listOfUsers) {
            LogDebug("User: " << user_id);
            std::string userStr = std::to_string(user_id);
            ...
            if (filter.appId.compare(SECURITY_MANAGER_ANY)) {
                ...
                PrivilegeDb::getInstance().GetUserApps(user_id, listOfApps);
            ...
            for (const std::string &appId : listOfApps) {
                ...
                PrivilegeDb::getInstance().GetAppPrivileges(appId, user_id, listOfPrivileges);
