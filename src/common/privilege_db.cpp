...
void PrivilegeDb::GetPrivateSharingForOwner(const std::string &appName,
...
void PrivilegeDb::GetPrivateSharingForTarget(const std::string &appName,
                                             std::map<std::string, std::vector<std::string>> &ownerSharing)
{
    try_catch<void>([&] {
        ...
        while (command->Step()) {
            ...
            ownerSharing[ownerAppName].push_back(path);
        }
    });
}
