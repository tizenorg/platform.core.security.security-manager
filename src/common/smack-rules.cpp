...
void SmackRules::generateSharedRORules(
        std::vector<std::pair<std::string, std::vector<std::string>>> tizen2XApps)
{
    LogDebug("Generating SharedRO rules");

    // Unload old rules from kernel
    {
        SmackRules rules;
        rules.loadFromFile(SMACK_RULES_SHARED_RO_PATH);
        rules.clear();
    }

    SmackRules rules;
    for (size_t i = 0; i < tizen2XApps.size(); ++i) {
        for (const std::string &appName : tizen2XApps[i].second) {
            std::string appLabel = SmackLabels::generateAppLabel(appName);
            for (size_t j = 0; j < tizen2XApps.size(); ++j) {
                if (j == i)
                    continue;
                const std::string &pkgName = tizen2XApps[j].first;
                const std::string sharedROLabel =
                    SmackLabels::generatePkgLabelOwnerRWothersRO(pkgName);

                rules.add(appLabel, sharedROLabel, SMACK_APP_CROSS_PKG_PERMS);
            }
        }
    }
...
void SmackRules::mergeRules()
{
    ...
    files.push_back(SMACK_RULES_SHARED_RO_PATH);
