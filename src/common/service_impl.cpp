...
int appInstall(const app_inst_req &req, uid_t uid)
{
    ...
    std::vector<std::string> appIdsInPkg;
    std::string uidstr;
    if (uid) {
        if (uid != req.uid) {
            LogError("User " << uid <<
    ...
    try {
        ...
        PrivilegeDb::getInstance().GetAppPrivileges(req.pkgId, req.uid, oldAppPrivileges);
        PrivilegeDb::getInstance().AddApplication(req.appId, req.pkgId, req.uid, pkgIdIsNew);
        PrivilegeDb::getInstance().UpdateAppPrivileges(req.appId, req.uid, req.privileges);
        /* Get all application ids in the package to generate rules withing the package */
        PrivilegeDb::getInstance().GetAppIdsForPkgId(req.pkgId, appIdsInPkg);
    ...
    LogDebug("Adding Smack rules for new appId: " << req.appId << " with pkgId: " << req.pkgId << ". Applications in package: " << appIdsInPkg.size());
    if (!SmackRules::installPackageRules(req.appId, req.pkgId, appIdsInPkg)) {
