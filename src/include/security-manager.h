...
enum app_install_path_type {
    ...
    //! RWX access for application packages coming from the same author
    SECURITY_MANAGER_PATH_TRUSTED,
 ...
 * This field is required for trusted paths only (SECURITY_MANAGER_PATH_TRUSTED).
 *
 * \param[in] Pointer handling app_inst_req structure
 * \param[in] Author's identifier
 * \param[in] Length of author's identifier
 * \return API return code or error code
 */
