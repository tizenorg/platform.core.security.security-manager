...
buffer digest_message(const void *message, size_t len)
{
    ...
    if(1 != EVP_DigestUpdate(ctx.get(), message, len))
...
std::string unify_author_id(const void *author_id, size_t len)
{
    // length of unified author id (in characters)
    static const size_t UNIFIED_LEN = 32;
    static_assert((UNIFIED_LEN & 0x3) == 0, "UNIFIED_LEN should be divisible by 4");
    // UNIFIED_LEN/4*3 because of Base64
    static const size_t DIGEST_LEN = (UNIFIED_LEN>>2)*3;

    buffer digest = digest_message(author_id, len);

    digest.resize(DIGEST_LEN,0);

    std::string b64 = base64(digest);

    // replace characters not allowed in smack label
...
int security_manager_app_inst_req_set_author_id(app_inst_req *p_req,
                                                const void *author_id,
                                                size_t author_id_len)
{
