...
int security_manager_groups_get(char ***groups, size_t *groups_count)
{
    ...
    return try_catch([&]() -> int {
        ...
        ClientOffline offlineMode(false);

        if (offlineMode.isOffline()) {
            int retval = ServiceImpl().policyGetGroups(vgroups);
            if (retval != SECURITY_MANAGER_SUCCESS) {
                LogError("Error in policyGetGroups. Error code: " << retval);
                return retval;
            }
        } else {
            //put data into buffer
...
int security_manager_groups_get_for_user(uid_t uid, char ***groups, size_t *groups_count)
{
    ...
    return try_catch([&]() -> int {
        ...
        if (offlineMode.isOffline()) {
            int retval = ServiceImpl().policyGroupsForUid(uid, vgroups);
            if (retval != SECURITY_MANAGER_SUCCESS) {
                LogError("Error in policyGetGroupsForUid. Error code: " << retval);
                return retval;
            }
        } else {
            MessageBuffer send, recv;
