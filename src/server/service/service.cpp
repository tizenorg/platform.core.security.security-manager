...
void Service::checkGlobalUser(uid_t &uid, std::string &cynaraUserStr)
{
    if (uid == 0 || uid == m_globaluid) {
        uid = m_globaluid;
...
Service::Service()
{
    m_globaluid = tzplatform_getuid(TZ_SYS_GLOBALAPP_USER);
}
...
{
    if (uid == 0)
        return false;//application should not be installed by root user any more
