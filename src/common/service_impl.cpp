...
bool ServiceImpl::installRequestAuthCheck(const Credentials &creds, const app_inst_req &req,
    std::string &appPath)
{
    static const std::map<app_install_type, const char*> installTypeName = {
        {SM_APP_INSTALL_LOCAL, "local"},
        {SM_APP_INSTALL_GLOBAL, "global"},
        {SM_APP_INSTALL_PRELOADED, "preloaded"}};

    auto installType = static_cast<app_install_type>(req.installationType);
    LogDebug("Installation type: " << installTypeName.at(installType));
    ...
    if (!getUserAppDir(req.uid, installType, appPath)) {
...
int ServiceImpl::appInstall(const Credentials &creds, app_inst_req &req)
{
    ...
    if (req.uid == 0)
        req.uid = getGlobalUserId();

    if (req.installationType == SM_APP_INSTALL_LOCAL)
        cynaraUserStr = std::to_string(static_cast<unsigned int>(req.uid));
    else
        cynaraUserStr = CYNARA_ADMIN_WILDCARD;

    if (!installRequestAuthCheck(creds, req, appPath)) {
...
int ServiceImpl::appUninstall(const Credentials &creds, app_inst_req &req, bool authorized)
{
    ...
    try {
        ...
        LogDebug("Uninstall parameters: appName: " << req.appName << ", pkgName: " << req.pkgName
                 << ", uid " << req.uid << ", generated smack label: " << smackLabel);
...
int ServiceImpl::userDelete(const Credentials &creds, uid_t uidDeleted)
{
    ...
    for (const auto &app : userApps) {
        ...
        req.uid = uidDeleted;
        req.appName = app;
        if (appUninstall(creds, req, true) != SECURITY_MANAGER_SUCCESS) {
