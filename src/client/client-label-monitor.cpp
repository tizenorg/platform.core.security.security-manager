...
static lib_retcode inotify_add_watch_full(int fd, const char* pathname, uint32_t mask, int *wd)
{
    ...
    if ((observed_fd == -1) && ((errsv = errno) != EEXIST)) {
        LogError("Creation of file" << pathname << "failed" << GetErrnoString(errsv));
    ...
    if (inotify_fd == -1) {
        errsv = errno;
        LogError("Inotify watch failed on file " << pathname << ": " << GetErrnoString(errsv));
        return SECURITY_MANAGER_ERROR_WATCH_ADD_TO_FILE_FAILED;
    }
    *wd = inotify_fd;
    return SECURITY_MANAGER_SUCCESS;
...
int security_manager_app_labels_monitor_init(app_labels_monitor **monitor)
{
    ...
    return try_catch([&] {
        ...
        *monitor = nullptr;
        ...
        if (ret == -1) {
            errsv = errno;
            LogError("Inotify init failed: " << GetErrnoString(errsv));
            return SECURITY_MANAGER_ERROR_WATCH_ADD_TO_FILE_FAILED;
        }
        m.get()->inotify = ret;
        ...
        if ((*monitor = m.release()) == nullptr) {
            LogError("Monitor is set to nullptr");
            ...
            if (monitor->global_labels_file_watch != -1) {
                ...
                if (ret == -1) {
                    ...
                    LogError("Inotify watch removal failed on file " <<
                            Config::SMACK_APPS_LABELS_GLOBAL_FILE << ": " << GetErrnoString(errsv));
                }
            }
...
int security_manager_app_labels_monitor_process(app_labels_monitor *monitor)
{
    ...
    return try_catch([&] {
        ...
        if (ret == -1) {
            errsv = errno;
            LogError("Ioctl on inotify descriptor failed: " << GetErrnoString(errsv));
            return SECURITY_MANAGER_ERROR_UNKNOWN;
        }
        ...
        for (int pos = 0; pos < avail;) {
            ...
            if (ret == -1) {
                errsv = errno;
                LogError("Inotify read failed: " << GetErrnoString(errsv));
                return SECURITY_MANAGER_ERROR_UNKNOWN;
            }
            pos += ret;
        }
