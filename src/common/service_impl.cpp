...
inline int validatePolicy(policy_entry &policyEntry, std::string uidStr, bool &forAdmin, CynaraAdminPolicy &cyap)
{
    ...
    if (policyEntry.currentLevel.empty()) { //for admin
        ...
        if (!policyEntry.maxLevel.compare("DELETE")) {
        ...
        if (!policyEntry.currentLevel.compare("DELETE")) {
            level = CYNARA_ADMIN_DELETE;
        } else {
            try {
        ...
        for (auto &entry : const_cast<std::vector<policy_entry>&>(policyEntries)) {
            ...
            if (ret == SECURITY_MANAGER_API_SUCCESS) {
                if (!forAdmin
                    ...
                    LogWarning("Not enough privilege to enforce admin policy");
                    return SECURITY_MANAGER_API_ERROR_ACCESS_DENIED;
                };
