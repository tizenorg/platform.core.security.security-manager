...
{
    const struct cynara_admin_policy *pp_policies[policies.size() + 1];

    for (unsigned i = 0; i < policies.size(); ++i)
        pp_policies[i] = dynamic_cast<const struct cynara_admin_policy *>(&policies[i]);

    pp_policies[policies.size()] = nullptr;

    checkCynaraAdminError(
        cynara_admin_set_policies(m_CynaraAdmin, pp_policies),
