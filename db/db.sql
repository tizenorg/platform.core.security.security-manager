...
CREATE TABLE IF NOT EXISTS privilege_version (
privilege_id INTEGER NOT NULL,
version_id INTEGER NOT NULL,
PRIMARY KEY (privilege_id, version_id),
FOREIGN KEY (privilege_id) REFERENCES privilege (privilege_id)
...
FOREIGN KEY (privilege_id, version_from_id) REFERENCES privilege_version (privilege_id, version_id)
FOREIGN KEY (privilege_mapping_id, version_to_id) REFERENCES privilege_version (privilege_id, version_id)
);
...
DROP VIEW IF EXISTS privilege_version_view;
CREATE VIEW privilege_version_view AS
SELECT
        version.name AS version,
        privilege.name AS privilege
FROM privilege_version
LEFT JOIN privilege USING (privilege_id)
LEFT JOIN version USING (version_id);

DROP VIEW IF EXISTS privilege_mapping_view;
CREATE VIEW privilege_mapping_view AS
SELECT
        version1.name AS version_from,
        version2.name AS version_to,
        privilege1.name AS privilege,
        privilege2.name AS privilege_mapping
...
BEGIN
        ...
        INSERT OR IGNORE INTO privilege_version(privilege_id, version_id) VALUES
            ((SELECT privilege_id FROM privilege WHERE name=NEW.privilege_name),
