...
int ServiceImpl::policyGroupsForUid(uid_t uid, std::vector<std::string> &groups)
{
    ...
    try {
        auto userType = CynaraAdmin::getInstance().UserType(uid);
        ...
        std::vector<std::tuple<std::string, std::string>> privileges;
        PrivilegeDb::getInstance().GetGroupsRelatedPrivileges(privileges);

        for (const auto &priv : privileges) {
            CynaraAdmin::getInstance().Check(CYNARA_ADMIN_ANY, uidStr, std::get<1>(priv),
                                             bucket, result, resultExtra, true);
            if (result == CYNARA_ADMIN_ALLOW)
                groups.push_back(std::get<0>(priv));
        }
    } catch (const CynaraException::Base &e) {
