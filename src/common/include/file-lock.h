...
public:
    ...
    FileLocker(const std::string &lockFile, bool canWait = false,
               unsigned int tryto = 0);
    FileLocker(bool lockNow, const std::string &lockFile, bool canWait = false,
               unsigned int tryto = 0);
    ~FileLocker();
...
private:
    ...
    void Init(bool lockNow, const std::string &lockFile, bool canWait = false,
              unsigned int tryto = 0);
};
