...
int security_manager_app_inst_req_new(app_inst_req **pp_req)
{
    ...
    if ((*pp_req)->uid == 0 || (*pp_req)->uid == globalUid)
        (*pp_req)->installationType = app_install_type::SM_APP_INSTALL_GLOBAL;
    else
        (*pp_req)->installationType = app_install_type::SM_APP_INSTALL_LOCAL;
...
int security_manager_app_install(const app_inst_req *p_req)
{
    ...
    return try_catch([&]() -> int {
        ...
        if (offlineMode.isOffline()) {
            ...
            retval = SecurityManager::ServiceImpl().appInstall(creds, *p_req);
        } else {
