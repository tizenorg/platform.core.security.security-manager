...
static lib_retcode get_app_and_pkg_id_from_smack_label(const std::string &label, char **pkg_id, char **app_id) {
    ...
    Serialization::Serialize(send, static_cast<int>(SecurityModuleCall::GET_PKGID_AND_APPID_FROM_SMACKLABEL),
        label);
    ...
    Deserialization::Deserialize(recv, appIdString);
    if (appIdString.empty()) {
        LogError("Unexpected empty appId");
        return SECURITY_MANAGER_ERROR_UNKNOWN;
    }
