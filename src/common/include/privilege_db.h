...
private:
    ...
    const std::map<StmtType, const char * const > Queries = {
        ...
        { StmtType::EAppIdExists, "SELECT app_id FROM app WHERE name=?" },
...
public:
     ...
     * @param[out] appExistsForOtherUser - return info if appId is in the database (installed for other user)
     * @exception DB::SqlConnection::Exception::InternalError on internal error
     */
    void RemoveApplication(const std::string &appId, uid_t uid, bool &pkgIdIsNoMore, bool &appExistsForOtherUser);
