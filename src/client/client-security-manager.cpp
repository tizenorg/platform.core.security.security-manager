...
int security_manager_get_smacklabel_pkgid(char **pkg_id, const char *label)
{
    ...
    LogDebug("security_manager_get_smacklabel_pkgid() called");

    return try_catch([&] {
        //checking parameters

        if (label == NULL) {
            LogError("security_manager_get_smacklabel_pkgid: label is NULL");
            return SECURITY_MANAGER_ERROR_INPUT_PARAM;
        }

        if (pkg_id == NULL) {
            LogError("security_manager_get_smacklabel_pkgid: pkg_id is NULL");
            return SECURITY_MANAGER_ERROR_INPUT_PARAM;
        }

        //put data into buffer
        Serialization::Serialize(send, static_cast<int>(SecurityModuleCall::APP_GET_SMACKLABEL_PKGID),
            std::string(label));
