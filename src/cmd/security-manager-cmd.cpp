...
static po::options_description getGenericOptions()
{
    ...
    opts.add_options()
         ...
         ("manage-users,m", po::value<std::string>()->required(), "add or remove user, parameter is either 'a' (for add) or 'r' (for remove)")
         ;
    return opts;
}
...
static int manageUserOperation(const struct user_req &req, std::string operation)
{
    ...
    if (operation == "a") {
        ...
        std::cout << "Manage user option requires either 'a'"
                " (for adding user) or 'r' (for removing user) argument" << std::endl;
        LogError("Manage user option requires either 'a'"
                " (for adding user) or 'r' (for removing user) argument");
        return EXIT_FAILURE;
    }
