...
enum class StmtType {
    ...
    EGetUserApps,
    EGetAppsInPkg,
    EGetPrivilegeVersions,
    EGetPrivilegeMappings,
    ECreateTempPrivilegeToMap,
...
private:
    ...
    const std::map<StmtType, const char * const > Queries = {
        ...
        { StmtType::EGetPrivilegeVersions, " SELECT version_name FROM privilege_version_view WHERE privilege_name = ? ORDER BY version_name" },
        { StmtType::EGetPrivilegeMappings, " SELECT privilege_mapping_name FROM privilege_mapping_view"
                                           " WHERE version_from_name=? AND version_to_name=? AND privilege_name=?"
                                           " ORDER BY privilege_name;"},
        { StmtType::EInsertPrivilegeToMap, " INSERT privilege_to_map(privilege_name) VALUES ?;"},
        { StmtType::EGetPrivilegesMappings, "SELECT privilege_mapping_name FROM privilege_mapping_view"
                                            " WHERE version_from_name=? AND version_to_name=?"
                                            " AND privilege_name IN (SELECT name FROM privilege_to_map);"},
...
public:
    ...
    /**
     * Retrieve versions privilege is available in
     *
     * @param privilege - privilege name
     * @param[out] version - vector of versions privilege is available in
     * @exception DB::SqlConnection::Exception::InternalError on internal error
     */
    void GetPrivilegeVersions(const std::string &privilege, std::vector<std::string> &version);

    /**
     * Retrieve privilege version
     *
