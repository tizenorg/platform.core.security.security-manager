...
void markPermissibleFileValid(int fd, const std::string &nameFile, bool valid)
{
    int ret;
    if (valid)
    ...
    if (ret == -1) {
        ...
        ThrowMsg(PermissibleSetException::FileChmodError, "Error at fchmod");
    }
}
...
void updatePermissibleFile(uid_t uid, int installationType)
{
    ...
    if (fflush(file.get()) != 0) {
        ...
        ThrowMsg(PermissibleSetException::FileFlushError, "Error at fflush");
    }
    if (fsync(fileno(file.get())) == -1) {
        LogError("Error at fsync " << nameFile << ": " << GetErrnoString(errno));
        ThrowMsg(PermissibleSetException::FileFsyncError, "Error at fsync");
    }
