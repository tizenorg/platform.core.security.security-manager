                                ...
                                std::string &userPkgDir)
{
    ...
    switch (static_cast<app_install_type>(installType)) {
...
int ServiceImpl::validateRequestAndGetPkgBasePath(const std::string &pkgName,
                                                  int installationType,
                                                  const uid_t &uid,
...
void ServiceImpl::labelPaths(const pkg_paths &paths,
                const std::string &pkgName,
                const std::string &pkgBasePath,
                const int authorId)
{
    if (!paths.empty())
...
int ServiceImpl::appInstall(const Credentials &creds, app_inst_req &&req)
{
    ...
    try {
        ...
        int ret = validateRequestAndGetPkgBasePath(req.pkgName,
    ...
    try {
        labelPaths(req.pkgPaths, req.pkgName, pkgBasePath, authorId);
