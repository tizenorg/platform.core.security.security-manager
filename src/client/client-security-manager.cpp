...
int security_manager_get_app_pkgid(char **pkg_id, const char *app_id)
{
    ...
    return try_catch([&] {
        ...
        //put data into buffer
        Serialization::Serialize(send, (int)SecurityModuleCall::APP_GET_PKGID);
        ...
        size_t len = pkgIdString.length();
        *pkg_id = static_cast<char *>(malloc(len + 1));
        if (*pkg_id == NULL) {
            LogError("Failed to allocate memory for pkgId");
            return SECURITY_MANAGER_ERROR_MEMORY;
        }

        pkgIdString.copy(*pkg_id, len);
        pkg_id[len] = '\0';
...
int security_manager_set_process_label_from_binary(const char *path)
{
    ...
    ret = SecurityManager::getLabelFromBinary(&smack_label, path);
