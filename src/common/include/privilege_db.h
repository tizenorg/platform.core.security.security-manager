...
private:
    ...
    const std::map<StmtType, const char * const > Queries = {
        ...
        { StmtType::EAddApplication, "INSERT INTO app_pkg_view (app_name, pkg_name, uid, Tizen_ver) VALUES (?, ?, ?, ?)" },
        ...
        { StmtType::EGetPkgIdAndVer, "SELECT pkg_name, Tizen_ver FROM app_pkg_view WHERE app_name = ?" },
        ...
        { StmtType::EGetAllTizen2XApps,  "SELECT name FROM app WHERE Tizen_ver LIKE '2.%%' AND name <> ?" },
        { StmtType::EGetAllTizen2XPackages,  "SELECT pkg_name FROM app_pkg_view WHERE Tizen_ver LIKE '2.%%' AND app_name <> ? GROUP BY pkg_name" },
