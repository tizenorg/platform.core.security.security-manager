                                            ...
                                            size_t *mappings_count)
{
    using namespace SecurityManager;
    (void)to_version;
    if (from_version == nullptr || privilege == nullptr ||
            privilege_mappings == nullptr || mappings_count == nullptr) {
        return SECURITY_MANAGER_ERROR_INPUT_PARAM;
    }
    return try_catch([&] {
        std::vector<std::string> privilegesToMap(1, privilege);
        std::vector<std::string> mappings;
        return get_privileges_mapping(from_version, to_version, privilegesToMap,
                                      privilege_mappings, mappings_count);
    });
                                             ...
                                             size_t *mappings_count)
{
    ...
    return try_catch([&] {
        std::vector<std::string> privilegesToMap(privileges, privileges + privileges_count);
        std::vector<std::string> mappings;
        return get_privileges_mapping(from_version, to_version, privilegesToMap,
