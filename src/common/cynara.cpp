    ...
    std::vector<CynaraAdminPolicy> *policies)
{
        ...
        "Error while getting list of policies for bucket: " + bucketName))
    {
        for (std::size_t i = 0; pp_policies[i] != nullptr; i++) {
            policies->push_back(CynaraAdminPolicy(*pp_policies[i]));

            free(pp_policies[i]->bucket);
            free(pp_policies[i]->client);
            free(pp_policies[i]->user);
            free(pp_policies[i]->privilege);
            free(pp_policies[i]->result_extra);
            free(pp_policies[i]);
        }
