...
{
    bool ret;
    ret = parseCommandOptions(argc, argv, "registeruser", getUserOptions(), vm);
    if (!ret)
        return ret;
    try {
        if (vm.count("uid"))
        ...
        std::cout << "Error while parsing user registeration arguments: " << e.what() <<
                  std::endl;
        LogError("Error while parsing user registeration arguments: " << e.what());
        ret = false;
    }
    return ret;
}
