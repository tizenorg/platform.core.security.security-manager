...
int AppInstall(PrivilegeDb *pdb, const app_inst_req &req, bool offlineMode)
{
    int ret = SECURITY_MANAGER_API_ERROR_SERVER_ERROR;

    LogDebug("ProcessAppInstall");
    if ((!offlineMode) && (nullptr == pdb)) {
        LogError("pdb cannot be null in normal mode.");
