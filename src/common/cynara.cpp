...
std::string CynaraAdmin::PrivacyDescription = "Ask user";
...
{
    auto op = [](const std::string &label,
        const std::string &user,
        const std::vector<std::string> &privileges,
        const std::string &bucket,
        int policy,
    ...
    {
        std::vector<CynaraAdminPolicy> tempPolicies;
        std::unordered_set<std::string> privilegesSet(privileges.begin(), privileges.end());
        CynaraAdmin::getInstance().ListPolicies(bucket, label, user, CYNARA_ADMIN_ANY, tempPolicies);

        // Compare previous policies with set of new requested privileges
        for (auto &policy : tempPolicies) {
        ...
        for (auto &p : tempPolicies)
            policies.push_back(CynaraAdminPolicy(p.client, p.user, p.privilege, p.result, p.bucket));
    ...
    op(label, user, privileges, Buckets.at(Bucket::MANIFESTS),
    ...
    if (user == CYNARA_ADMIN_WILDCARD) {
        ...
        ListUsers(users);
        for (uid_t id : users) {
            op(label, std::to_string(id), privacyPrivileges, Buckets.at(Bucket::PRIVACY_MANAGER),
                askUserPolicy, policies);
        }
    } else {
        // local single user installation, do it only for that particular user
        op(label, user, privacyPrivileges, Buckets.at(Bucket::PRIVACY_MANAGER),
            askUserPolicy, policies);
    }
        ...
        std::function <bool(const char*)> isPrivacy)
{
    ...
    CynaraAdmin::getInstance().ListPolicies(CynaraAdmin::Buckets.at(Bucket::MANIFESTS),
        CYNARA_ADMIN_ANY, CYNARA_ADMIN_WILDCARD, CYNARA_ADMIN_ANY, appPolicies);

    for (CynaraAdminPolicy &p : appPolicies)
        if (isPrivacy(p.privilege))
            policies.push_back(CynaraAdminPolicy(p.client,
                userStr,
                p.privilege,
                askUserPolicy,
