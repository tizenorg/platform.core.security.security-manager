...
private:
    ...
    const std::map<QueryType, const char * const > Queries = {
        ...
        { QueryType::EGetPrivilegeGroups, " SELECT grp_name FROM privilege_group_view WHERE privilege_name = ?" },
    };
...
public:
     ...
     * @param[out] grp_name - list of group names assigned to the privilege
     * @exception DB::SqlConnection::Exception::InternalError on internal error
     */
    void GetPrivilegeGroups(const std::string &privilege,
        std::vector<std::pair<gid_t, std::string>> &grp_names);
