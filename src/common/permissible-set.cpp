...
void getUserAppLabels(uid_t uid, std::vector<std::string> &appLabels)
 {
...
std::string getPerrmissibleFileLocation(int installationType)
{
    if ((installationType == SM_APP_INSTALL_GLOBAL)
        ...
        return std::string(
                tzplatform_mkpath(TZ_SYS_RW_APP, Config::SMACK_APPS_LABELS_GLOBAL_FILE.c_str()));
    return std::string(tzplatform_mkpath(TZ_USER_APP, Config::SMACK_APPS_LABELS_USER_FILE.c_str()));
...
void updatePermissibleFile(uid_t uid, int installationType) {
    try {
        ...
        LogDebug("updatePermissibleFile throws an exception");
        throw;
    }
}
