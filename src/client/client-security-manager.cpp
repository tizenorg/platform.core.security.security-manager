...
policy_entry *allocate_and_copy_entries(const std::vector<std::string> &names,
                                        ...
                                        const std::vector<int> &curValues)
{
    if (names.size() != maxValues.size()
     || names.size() != curValues.size()) {
    ...
    for(size_t i = 0; i < names.size(); i++) {
        ...
        result[i].max_value = maxValues.at(i);
        result[i].current = curValues.at(i);
    }
                                     ...
                                     int value)
{
    if (!p_req
     ...
     || user_type < SM_USER_TYPE_ANY || user_type >= SM_USER_TYPE_END
    ...
    try {
        p_req->units.push_back(
                SecurityManager::policyUpdateUnit(uid_str, app_id, privilege,
...
int security_manager_policy_update_req_send(policy_update_req *p_req)
{
    ...
    return try_catch([&] {
        ...
        for (auto &unit : p_req->units) {
            Serialization::Serialize(send, unit.userId);
            Serialization::Serialize(send, unit.appId);
            Serialization::Serialize(send, unit.privilege);
            Serialization::Serialize(send, unit.userType);
            Serialization::Serialize(send, unit.value);
        }
...
SECURITY_MANAGER_API
int security_manager_policy_get_user_apps(uid_t uid,
                                          ...
                                          size_t *p_size)
{
    ...
    return try_catch([&] {
        ...
        case SECURITY_MANAGER_API_SUCCESS: {
            ...
            Deserialization::Deserialize(recv, maxValues);
            Deserialization::Deserialize(recv, curValues);

            if (apps.size() != maxValues.size()    /* all sizes must be equal! */
             || apps.size() != curValues.size()) {
                LogError("Received malformed data, sizes of vectors are different!");
                return SECURITY_MANAGER_ERROR_UNKNOWN;
            }
            *pp_apps_policy = allocate_and_copy_entries(apps, maxValues, curValues);
            *p_size = apps.size();
...
SECURITY_MANAGER_API
int security_manager_policy_get_apps_for_self(policy_entry **pp_apps_policy, size_t *p_size)
{
    return security_manager_policy_get_user_apps(getuid(), pp_apps_policy, p_size);
}

SECURITY_MANAGER_API
int security_manager_policy_get_global_apps(policy_entry **pp_apps_policy, size_t *p_size)
{
    return security_manager_policy_get_user_apps(tzplatform_getuid(TZ_SYS_GLOBALAPP_USER),
                                         ...
                                         size_t *p_size)
{
    ...
    return try_catch([&] {
        ...
        case SECURITY_MANAGER_API_SUCCESS: {
            ...
            if (privileges.size() != maxValues.size()    /* all sizes must be equal! */
             || privileges.size() != curValues.size()) {
                LogError("Received malformed data, sizes of vectors are different!");
                return SECURITY_MANAGER_ERROR_UNKNOWN;
            }
            *pp_privs_policy = allocate_and_copy_entries(privileges, maxValues, curValues);
...
int security_manager_policy_get_for_self(const char *app_id,
                                         ...
                                         size_t *p_size)
{
    return security_manager_policy_get_for_user(tzplatform_getuid(TZ_SYS_GLOBALAPP_USER),
                                                app_id, privilege,
