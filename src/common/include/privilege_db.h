...
private:
    ...
    const std::map<StmtType, const char * const > Queries = {
        { StmtType::EGetPkgPrivileges, "SELECT DISTINCT privilege_name FROM user_app_privilege_view WHERE pkg_name=? AND uid=? ORDER BY privilege_name"},
        { StmtType::EGetAppPrivileges, "SELECT DISTINCT privilege_name FROM user_app_privilege_view WHERE app_name=? AND uid=? ORDER BY privilege_name"},
        ...
        { StmtType::EAddAppPrivileges, "INSERT INTO user_app_privilege_view (app_name, uid, privilege_name) VALUES (?, ?, ?)" },
        { StmtType::ERemoveAppPrivileges, "DELETE FROM user_app_privilege_view WHERE app_name=? AND uid=?" },
        ...
        { StmtType::EGetAllTizen2XPackages,  "SELECT DISTINCT pkg_name FROM user_app_pkg_view WHERE version LIKE '2.%%' AND app_name <> ?" },
