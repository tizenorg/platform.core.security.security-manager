...
set_smack()
{
### Set proper Smack labels on the database files
	chsmack -a System "$db"
	chsmack -a System "$db_journal"
}
...
if [ ! -e $db ]
then
	echo "Database not found, creating initial database"
...
[ $db_version -eq $db_version_new ] && echo "Database is already up to date. Aborting." && exit
