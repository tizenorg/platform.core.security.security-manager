...
static filePtr openAndLockLabelFile(const int installationType, const std::string &user,
    ...
    if (!file) {
        errsv = errno;
        LogError("Unable to open file " << labelFile << ": " << strerror(errsv));
        return filePtr(nullptr, nullptr);
    }
    ...
    if (ret == -1) {
        errsv = errno;
        LogError("Unable to lock file " << labelFile << ": " << strerror(errsv));
        return filePtr(nullptr, nullptr);
    }
    return file;
}
         ...
         int installationType)
{
    ...
    if (fprintf(file.get(), "%s\n", label.c_str()) < 0) {
        errsv = errno;
        LogError("Unable to fprintf() to file " << labelFile << ": " << strerror(errsv));
        fflush(file.get());
        ...
        int installationType)
{
    ...
    do {
        ...
        case -1:
            if (feof(file.get()))
                break;
            errsv = errno;
            LogError("Failure while reading file " << labelFile << ": " << strerror(errsv));
    ...
    if (TEMP_FAILURE_RETRY(ftruncate(fileno(file.get()), 0) == -1)) {
        errsv = errno;
        LogError("Cannot truncate labels file " << labelFile << ": " << strerror(errsv));
        return false;
    }
    rewind(file.get());
    for (unsigned int i = 0; i < labels.size(); ++i)
        if (fprintf(file.get(), "%s\n", labels[i]) == -1) {
            errsv = errno;
            LogError("Failure while writing to file " << labelFile << ": " << strerror(errsv));
    ...
    if (fsync(fileno(file.get())) == -1) {
        errsv = errno;
        LogError("Failure while fsync() file " << labelFile << ": " << strerror(errsv));
        TEMP_FAILURE_RETRY(ftruncate(fileno(file.get()), 0));
        return false;
    }
    return true;
        ...
        PtrVector<char> &labels)
{
    ...
    if (!file) {
        errsv = errno;
        LogError("Unable to open file " << label_file << ": " << strerror(errsv));
        return SECURITY_MANAGER_ERROR_FILE_OPEN_FAILED;
    }
    ...
    if (r == -1) {
        errsv = errno;
        LogError("Unable to lock file " << label_file << ": " << strerror(errsv));
        return SECURITY_MANAGER_ERROR_FILE_OPEN_FAILED;
    }
    ...
    do {
        ...
        case -1:
            if (feof(file.get()))
                break;
            errsv = errno;
            LogError("Failure while reading file " << label_file << ": " << strerror(errsv));
            return SECURITY_MANAGER_ERROR_FILE_OPEN_FAILED;
        default:
            if (buf[ret - 1] == '\n')
                buf[ret - 1] = '\0';
