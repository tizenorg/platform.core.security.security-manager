...
int ServiceImpl::policyUpdate(const Credentials &creds, const std::vector<policy_entry> &policyEntries)
{
    ...
    try {
        ...
        // Check privileges
        if (!Cynara::getInstance().check(creds.label,
                adminRequired ? Config::PRIVILEGE_POLICY_ADMIN : Config::PRIVILEGE_POLICY_USER,
                uidStr, pidStr)) {
            LogError("Not enough privilege to enforce " <<
                (adminRequired ? "admin" ? "user") << " policy");
