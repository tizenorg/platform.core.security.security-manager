...
int appInstall(const app_inst_req &req, uid_t uid, bool isSlave)
{
    ...
    try {
        ...
        appLabel = generateLabelForZone(SmackLabels::generateAppLabel(req.appId), zoneId);
        /* NOTE: we don't use pkgLabel here, but generate it for pkgId validation */
        pkgLabel = generateLabelForZone(SmackLabels::generatePkgLabel(req.pkgId), zoneId);
...
int appUninstall(const std::string &appId, uid_t uid, bool isSlave)
{
    ...
    try {
        ...
        if (!PrivilegeDb::getInstance().GetAppPkgId(appId, pkgId)) {
            ...
            smackLabel = generateLabelForZone(SmackLabels::generateAppLabel(appId), zoneId);
