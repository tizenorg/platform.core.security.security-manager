...
// HOST_NAME should be visible outside to other modules
...
{
    if (zoneName.empty() || zoneName == HOST_NAME) {
        return label;
    }
...
bool zoneSmackLabelMap(const std::string &hostLabel, const std::string &zoneLabel)
{
    (void) hostLabel;
    (void) zoneLabel;
...
bool zoneSmackLabelUnmap(const std::string &label)
{
    (void) label;
