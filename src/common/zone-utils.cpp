...
std::string generateLabelForZone(const std::string &label, const std::string &zoneName)
{
    if (zoneName.empty() || zoneName == "host") {
        LogInfo("Running in host - label unchanged.");
        return label;
    }
